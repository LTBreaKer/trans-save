...
echo db:5432:$TAG_GAME_DB:$POSTGRES_USER:$POSTGRES_PASSWORD > ~/.pgpass

chmod 0600 ~/.pgpass

until PGPASSFILE=~/.pgpass psql -h db -U $POSTGRES_USER -d $TAG_GAME_DB -c "SELECT 1" >/dev/null 2>&1; do
...
python /tag_game_db/manage.py makemigrations
python /tag_game_db/manage.py migrate
